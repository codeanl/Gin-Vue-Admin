...
// 根据接口ID获取接口列表
func GetApisById(apiIds []uint) ([]*model.Api, error) {
	var apis []*model.Api
...
// 获取接口列表
func GetApis(method, path, category, creator string, pageNum, pageSize int) ([]*model.Api, int64, error) {
	var list []*model.Api
...
func GetApiTree() ([]*response.ApiTreeDto, error) {
...
// 批量删除接口
func BatchDeleteApiByIds(apiIds []uint) error {
	apis, err := GetApisById(apiIds)
	if err != nil {
...
// 更新接口
func UpdateApiById(apiId uint, api *model.Api) error {
	// 根据id获取接口信息
