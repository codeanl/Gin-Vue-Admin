...
func InitLogger() {
	now := time.Now()
	infoLogFileName := fmt.Sprintf("%s/info/%04d-%02d-%02d.log", conf.Conf.Logs.Path, now.Year(), now.Month(), now.Day())
	errorLogFileName := fmt.Sprintf("%s/error/%04d-%02d-%02d.log", conf.Conf.Logs.Path, now.Year(), now.Month(), now.Day())
	...
	infoFileWriteSyncer := zapcore.AddSync(&lumberjack.Logger{
		...
		MaxSize:    conf.Conf.Logs.MaxSize,    //文件大小限制,单位MB
		MaxAge:     conf.Conf.Logs.MaxAge,     //日志文件保留天数
		MaxBackups: conf.Conf.Logs.MaxBackups, //最大保留日志文件数量
		LocalTime:  false,
		Compress:   conf.Conf.Logs.Compress, //是否压缩处理
	})
